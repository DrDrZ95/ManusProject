...
public static class RedocExtensions
{
    ...
    public static IApplicationBuilder UseRedocDocumentation(this IApplicationBuilder app)
    {
        ...
        app.UseReDoc(options =>
        {
            ...
            // ReDoc typically shows one version at a time. By default, we point to the fixed /openapi.json endpoint.
            // ReDoc 通常一次显示一个版本。默认情况下，我们指向固定的 /openapi.json 端点。
            var latestVersion = provider.ApiVersionDescriptions.OrderByDescending(v => v.ApiVersion).FirstOrDefault();
            
            if (latestVersion != null)
            {
                // Points to the fixed openapi.json endpoint which handles redirection to the latest version.
                // 指向固定的 openapi.json 端点，该端点负责处理对最新版本的重定向。
